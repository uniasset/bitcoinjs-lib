...
function p2tr(a, opts) {
    ...
    const _hashTree = lazy.value(() => {
        if (a.scriptTree)
            return bip341_1.toHashTree(a.scriptTree);
        if (a.hash)
            return { hash: a.hash };
        return;
    });
    ...
    lazy.prop(o, 'hash', () => {
        ...
        if (w && w.length > 1) {
            ...
            const leafHash = bip341_1.tapleafHash({ output: script, version: leafVersion });
            return bip341_1.rootHashFromPath(controlBlock, leafHash);
        }
        return null;
    });
    lazy.prop(o, 'output', () => {
    ...
    lazy.prop(o, 'pubkey', () => {
        ...
        if (o.internalPubkey) {
            const tweakedKey = bip341_1.tweakKey(o.internalPubkey, o.hash);
            if (tweakedKey)
                return tweakedKey.x;
        }
    });
    ...
    lazy.prop(o, 'witness', () => {
        ...
        if (hashTree && a.redeem && a.redeem.output && a.internalPubkey) {
            const leafHash = bip341_1.tapleafHash({
                output: a.redeem.output,
                version: o.redeemVersion,
            });
            const path = bip341_1.findScriptPath(hashTree, leafHash);
            if (!path)
                return;
            const outputKey = bip341_1.tweakKey(a.internalPubkey, hashTree.hash);
            if (!outputKey)
                return;
    ...
    if (opts.validate) {
        ...
        if (a.internalPubkey) {
            const tweakedKey = bip341_1.tweakKey(a.internalPubkey, o.hash);
        ...
        if (pubkey && pubkey.length) {
            if (!ecc_lib_1.getEccLib().isXOnlyPoint(pubkey))
        ...
        if (a.redeem && a.redeem.output && hashTree) {
            ...
            if (!bip341_1.findScriptPath(hashTree, leafHash))
        ...
        if (witness && witness.length) {
            if (witness.length === 1) {
                ...
                if (!ecc_lib_1.getEccLib().isXOnlyPoint(internalPubkey))
                    throw new TypeError('Invalid internalPubkey for p2tr witness');
                const leafVersion = controlBlock[0] & types_1.TAPLEAF_VERSION_MASK;
                const script = witness[witness.length - 2];
                const leafHash = bip341_1.tapleafHash({ output: script, version: leafVersion });
                const hash = bip341_1.rootHashFromPath(controlBlock, leafHash);
                const outputKey = bip341_1.tweakKey(internalPubkey, hash);
                if (!outputKey)
