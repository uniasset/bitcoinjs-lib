...
function tapScriptFinalizer(inputIndex, input, tapLeafHashToFinalize) {
    ...
    try {
        ...
        return { finalScriptWitness: psbtutils_1.witnessStackToScriptWitness(witness) };
    }
    catch (err) {
...
function isTaprootInput(input) {
    return (input &&
        !!(input.tapInternalKey ||
            ...
            (input.witnessUtxo && psbtutils_1.isP2TR(input.witnessUtxo.script))));
}
...
function isTaprootOutput(output, script) {
    return (output &&
        !!(output.tapInternalKey ||
            ...
            (script && psbtutils_1.isP2TR(script))));
}
...
function tweakInternalPubKey(inputIndex, input) {
    ...
    const outputKey = tapInternalKey && bip341_1.tweakKey(tapInternalKey, input.tapMerkleRoot);
    if (!outputKey)
...
function checkTaprootInputForSigs(input, action) {
    ...
    return sigs.some(sig => psbtutils_2.signatureBlocksAction(sig, decodeSchnorrSignature, action));
}
...
function isTapLeafInTree(tapLeaf, merkleRoot) {
    ...
    const rootHash = bip341_1.rootHashFromPath(tapLeaf.controlBlock, leafHash);
...
function addPubkeyPositionInScript(script, tss) {
    return Object.assign({
        positionInScript: psbtutils_1.pubkeyPositionInScript(tss.pubkey, script),
    }, tss);
}
/**
