...
class Psbt {
    ...
    addInput(inputData) {
        ...
        bip371_1.checkTaprootInputFields(inputData, inputData, 'addInput');
    ...
    addOutput(outputData) {
        ...
        bip371_1.checkTaprootOutputFields(outputData, outputData, 'addOutput');
        const c = this.__CACHE;
    ...
    finalizeInput(inputIndex, finalScriptsFunc) {
        ...
        if (bip371_1.isTaprootInput(input))
    ...
    _finalizeTaprootInput(inputIndex, input, tapLeafHashToFinalize, finalScriptsFunc = bip371_1.tapScriptFinalizer) {
        ...
        if (input.tapKeySig) {
            ...
            const finalScriptWitness = psbtutils_1.witnessStackToScriptWitness(payment.witness);
            this.data.updateInput(inputIndex, { finalScriptWitness });
        }
        else {
    ...
    validateSignaturesOfTaprootInput(inputIndex, validator, pubkey) {
        ...
        if (typeof validator !== 'function')
            throw new Error('Need validator function to validate signatures');
        pubkey = pubkey && bip371_1.toXOnly(pubkey);
    ...
    _signTaprootInput(inputIndex, input, keyPair, tapLeafHashToSign, allowedSighashTypes = [transaction_1.Transaction.SIGHASH_DEFAULT]) {
        ...
        const tapKeySig = hashesForSig
            .filter(h => !h.leafHash)
            .map(h => bip371_1.serializeTaprootSignature(keyPair.signSchnorr(h.hash), input.sighashType))[0];
        const tapScriptSig = hashesForSig
            .filter(h => !!h.leafHash)
            .map(h => ({
            pubkey: bip371_1.toXOnly(keyPair.publicKey),
            signature: bip371_1.serializeTaprootSignature(keyPair.signSchnorr(h.hash), input.sighashType),
            leafHash: h.leafHash,
        }));
        if (tapKeySig) {
    ...
    async _signTaprootInputAsync(inputIndex, input, keyPair, tapLeafHash, sighashTypes = [transaction_1.Transaction.SIGHASH_DEFAULT]) {
        ...
        if (tapKeyHash) {
            const tapKeySigPromise = Promise.resolve(keyPair.signSchnorr(tapKeyHash.hash)).then(sig => {
                return { tapKeySig: bip371_1.serializeTaprootSignature(sig, input.sighashType) };
            });
            ...
            const tapScriptSigPromises = tapScriptHashes.map(tsh => {
                return Promise.resolve(keyPair.signSchnorr(tsh.hash)).then(signature => {
                    const tapScriptSig = [
                        {
                            pubkey: bip371_1.toXOnly(keyPair.publicKey),
                            signature: bip371_1.serializeTaprootSignature(signature, input.sighashType),
    ...
    updateInput(inputIndex, updateData) {
        ...
        bip371_1.checkTaprootInputFields(this.data.inputs[inputIndex], updateData, 'updateInput');
    ...
    updateOutput(outputIndex, updateData) {
        ...
        bip371_1.checkTaprootOutputFields(outputData, updateData, 'updateOutput');
...
function checkInputsForPartialSig(inputs, action) {
    inputs.forEach(input => {
        const throws = bip371_1.isTaprootInput(input)
            ? bip371_1.checkTaprootInputForSigs(input, action)
            : psbtutils_1.checkInputForSig(input, action);
        if (throws)
...
function checkScriptForPubkey(pubkey, script, action) {
    if (!psbtutils_1.pubkeyInScript(pubkey, script)) {
...
function prepareFinalScripts(script, scriptType, partialSig, isSegwit, isP2SH, isP2WSH) {
    ...
    if (isSegwit) {
        if (p2wsh) {
            finalScriptWitness = psbtutils_1.witnessStackToScriptWitness(p2wsh.witness);
        }
        else {
            finalScriptWitness = psbtutils_1.witnessStackToScriptWitness(payment.witness);
        }
        if (p2sh) {
...
function getHashForSig(inputIndex, input, cache, forValidate, sighashTypes) {
    ...
    else if (psbtutils_1.isP2WPKH(meaningfulScript)) {
...
function getAllTaprootHashesForSig(inputIndex, input, inputs, cache) {
    ...
    if (input.tapInternalKey) {
        const outputKey = bip371_1.tweakInternalPubKey(inputIndex, input);
...
function getTaprootHashesForSig(inputIndex, input, inputs, pubkey, cache, tapLeafHashToSign, allowedSighashTypes) {
    ...
    if (input.tapInternalKey && !tapLeafHashToSign) {
        const outputKey = bip371_1.tweakInternalPubKey(inputIndex, input);
        if (bip371_1.toXOnly(pubkey).equals(outputKey)) {
            const tapKeyHash = unsignedTx.hashForWitnessV1(inputIndex, signingScripts, values, sighashType);
            hashes.push({ pubkey, hash: tapKeyHash });
        }
    }
    const tapLeafHashes = (input.tapLeafScript || [])
        .filter(tapLeaf => psbtutils_1.pubkeyInScript(pubkey, tapLeaf.script))
        .map(tapLeaf => {
        const hash = bip341_1.tapleafHash({
...
function getScriptFromInput(inputIndex, input, cache) {
    ...
    if (input.witnessScript || psbtutils_1.isP2WPKH(res.script)) {
        res.isSegwit = true;
    }
    return res;
}
...
function pubkeyInInput(pubkey, input, inputIndex, cache) {
    ...
    return psbtutils_1.pubkeyInScript(pubkey, meaningfulScript);
}
...
function getMeaningfulScript(script, index, ioType, redeemScript, witnessScript) {
    const isP2SH = psbtutils_1.isP2SHScript(script);
    const isP2SHP2WSH = isP2SH && redeemScript && psbtutils_1.isP2WSHScript(redeemScript);
    const isP2WSH = psbtutils_1.isP2WSHScript(script);
...
function checkInvalidP2WSH(script) {
    if (psbtutils_1.isP2WPKH(script) || psbtutils_1.isP2SHScript(script)) {
        throw new Error('P2WPKH or P2SH can not be contained within P2WSH');
    }
}
function classifyScript(script) {
    if (psbtutils_1.isP2WPKH(script))
        return 'witnesspubkeyhash';
    if (psbtutils_1.isP2PKH(script))
        return 'pubkeyhash';
    if (psbtutils_1.isP2MS(script))
        return 'multisig';
    if (psbtutils_1.isP2PK(script))
        return 'pubkey';
    return 'nonstandard';
}
function range(n) {
